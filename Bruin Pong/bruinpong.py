...
def getRandom():
    ...
    #MARS CHANGE BACK to 4 AFTER DEMO
    if level == "Mars":
        list1 = [1, 2,]

    #CHANGE JUPITER to 4 AFTER DEMO
    if level == "Jupiter":
        list1 = [1, 2, 3,]
    if level == "Venus":
        list1 = [1, 2, 3, 4]
...
while True:
    for event in pygame.event.get():
        ...
        if single_mode_active or multiplayer_mode_active:
            ...
            elif msg_receieved and not is_voice:
                # msg_receieved = 0
    ...
    if single_mode_active:
        ...
        if levelpast == "unchanged":
            ...
            bg_music.stop()
            level_levels = getsky(level,levelpast, sky_surface, bg_music,levelNumber)
